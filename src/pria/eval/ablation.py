...
def matrix() -> list[RetrievalConfig]:
    return [
        # --- single retrievers ------------------------------------------------
        BASE.variant("R01 bm25-only", use_dense=False, fusion="none", rerank=False, metadata_filter=False, quantize=False, semantic_cache=False),
        BASE.variant("R02 dense-only-hash", use_bm25=False, embedder="hash", fusion="none", rerank=False, metadata_filter=False, quantize=False, semantic_cache=False),
        BASE.variant("R03 dense-only-lsa", use_bm25=False, fusion="none", rerank=False, metadata_filter=False, quantize=False, semantic_cache=False),
        # --- fusion -----------------------------------------------------------
        BASE.variant("R04 linear a=0.3", fusion="linear", linear_alpha=0.3, rerank=False, metadata_filter=False, quantize=False, semantic_cache=False),
        BASE.variant("R05 linear a=0.5", fusion="linear", linear_alpha=0.5, rerank=False, metadata_filter=False, quantize=False, semantic_cache=False),
        BASE.variant("R06 linear a=0.7", fusion="linear", linear_alpha=0.7, rerank=False, metadata_filter=False, quantize=False, semantic_cache=False),
        BASE.variant("R07 rrf k=10", rrf_k=10, rerank=False, metadata_filter=False, quantize=False, semantic_cache=False),
        BASE.variant("R08 rrf k=60", rrf_k=60, rerank=False, metadata_filter=False, quantize=False, semantic_cache=False),
        BASE.variant("R09 rrf k=120", rrf_k=120, rerank=False, metadata_filter=False, quantize=False, semantic_cache=False),
        # --- filtering and reranking -----------------------------------------
        BASE.variant("R10 rrf + metadata filter", quantize=False, semantic_cache=False),
        BASE.variant("R11 rrf + rerank", rerank=True, metadata_filter=False, quantize=False, semantic_cache=False),
        BASE.variant("R12 rrf + filter + rerank", rerank=True, quantize=False, semantic_cache=False),
        BASE.variant("R13 rerank depth 20", rerank=True, rerank_depth=20, quantize=False, semantic_cache=False),
        BASE.variant("R14 rerank depth 60", rerank=True, rerank_depth=60, quantize=False, semantic_cache=False),
        # --- query expansion (negative result) --------------------------------
        BASE.variant("R15 + HyDE expansion", use_hyde=True, quantize=False, semantic_cache=False),
        # --- serving ----------------------------------------------------------
        BASE.variant("R16 + int8 quantization", semantic_cache=False),
        BASE.variant("R17 final (rrf + filter, quantized + cached)"),
    ]
...
def to_markdown(rows: list[dict[str, Any]]) -> str:
    headers = ["config", "ndcg@10", "mrr", "recall@10", "lexical_code_mrr", "page_recall@1", "p50_ms", "vector_bytes"]
    out = ["| " + " | ".join(headers) + " |", "|" + "---|" * len(headers)]
    for row in rows:
        out.append("| " + " | ".join(str(row.get(h, "")) for h in headers) + " |")

    baseline = next((r for r in rows if r["config"].startswith("R01")), None)
    final = next((r for r in rows if r["config"].startswith("R17")), None)
    hyde = next((r for r in rows if "HyDE" in r["config"]), None)
    pre_hyde = next((r for r in rows if r["config"].startswith("R10")), None)
